...
Конфигурируйте сервисы через метод `configuration`, обычно расположенный в базовом классе.
...
Основывайте пользовательские хелперы для `input` на опциях `must` и `prepare`.
...
Основывайте пользовательские хелперы для `internal` на опции `must`.
...
Основывайте пользовательские хелперы для `output` на опции `must`.
...
Добавляйте альтернативы для `make` через конфигурацию `action_aliases`.
...
Реализуйте сокращения для `make` через конфигурацию `action_shortcuts`.

Значения заменяют `make` и служат префиксом метода инстанса.
...
Методы предикаты для всех атрибутов включены по умолчанию.
Отключайте их при необходимости.
